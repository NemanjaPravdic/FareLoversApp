...
namespace ResvoyageMobileApp.Services.User
{
    ...
    {
        public async Task<UserRegistrationResponse> EditUserAsync(UserViewModel userViewModel)
        {
            var request = TransformUserViewModel(userViewModel);
            var token = await GetToken();
            var restRequest = new RestRequest("/api/v1/users/"+ userViewModel.Username + "/update", Method.POST);
        ...
        private UserRequest TransformUserViewModel(UserViewModel userViewModel)
        {
            return new UserRequest
            {
                Username = userViewModel.Username,
                Password = string.IsNullOrEmpty(userViewModel.Password) ? userViewModel.OldPassword : userViewModel.Password,
